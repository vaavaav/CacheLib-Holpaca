...
EXTERNAL_DIR="$SCRIPT_DIR/cachelib/external"

JOBS=1
PREFIX=$SCRIPT_DIR/opt/cachelib
...
CMAKE_FLAGS=(
  ...
  "-DCMAKE_MODULE_PATH=$PREFIX/lib/cmake/;$SCRIPT_DIR/cachelib/cmake/"
...
if [ $build_deps -eq 1 ]; then
  ...
  do
    ...
    if [ ! -d "$EXTERNAL_DIR/$1" ]; then
      ...
      git clone $GIT_FLAGS "$5" "$EXTERNAL_DIR/$1" || (rm -r "$EXTERNAL_DIR/$1" ; die "failed to clone $1")
    fi
  done
  # Build dependencies
  for d in "${dependencies[@]}"
  do
    set -- $d
    cmake "${CMAKE_FLAGS[@]}" "${@:7}" -B"build-$1" -S"$EXTERNAL_DIR/$1/$6" \
    && make -C"build-$1" -j"$JOBS" install \
    || die "failed to build $1" 
  done
fi

cmake "${CMAKE_FLAGS[@]}" -B"build-cachelib" -S"$SCRIPT_DIR/cachelib" \
  && make -C"build-cachelib" -j$JOBS install \
